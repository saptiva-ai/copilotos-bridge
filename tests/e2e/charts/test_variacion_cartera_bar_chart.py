#!/usr/bin/env python3
"""
E2E Test — Variacion de Cartera Total: Bar Chart Pipeline

Sends the exact multi-bank variation prompt and validates every component
of the response: chart type, period parsing, bank coverage, coloring
(INVEX red, rest grey), zeroline, table_data, and anti-fabrication guards.

Source: SPEC variacion-cartera-bar-chart (2026-02-12)

Prompt under test:
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025.
     Compara la cartera total entre el periodo inicial y el periodo final entre
     los bancos: MONEX, INVEX, BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME,
     BANSI, VE POR MÁS Y BANCO BASE. Presenta el dato del periodo inicial, el
     dato del periodo final y la variación. Done la variacion es =
     (periodo actual / periodo inicial -1). Haz una gráfica de barras donde se
     vea la variacion graficada y marca a invex de color rojo. Así como una
     tabla con: Banco | Cartera Total 2024 | Cartera Total 2025 | % Variación"

Usage:
    python tests/e2e/charts/test_variacion_cartera_bar_chart.py
"""
...
# The exact prompt from the spec
PROMPT = (
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025. "
    "Compara la cartera total entre el periodo inicial y el periodo final entre "
    "los bancos: MONEX, INVEX, BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME, "
    "BANSI, VE POR MÁS Y BANCO BASE. "
...
# Phrases that indicate the LLM is denying data that IS present in the chart.
# This is the "text/chart contradiction" bug: chart correct, text says no data.
...
# ══════════════════════════════════════════════════════════════════════════════
# Chart helpers (reused from feedback_replay pattern)
# ══════════════════════════════════════════════════════════════════════════════


def _get_plotly_config(resp: dict[str, Any]) -> dict[str, Any] | None:
    """Extract plotly_config from bank_chart."""
...
def _get_first_trace(resp: dict[str, Any]) -> dict[str, Any] | None:
    """Extract the first data trace from plotly_config."""
...
def _extract_trace_names(resp: dict[str, Any]) -> list[str]:
    ...
    traces = plotly.get("data", [])
    # For bar charts, banks are in trace.y
...
def _get_layout(resp: dict[str, Any]) -> dict[str, Any] | None:
    """Extract layout from plotly_config."""
    plotly = _get_plotly_config(resp)
    if not plotly:
        return None
...
def _check_chart_exists(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    if chart_status not in ("success", ""):
        # chart_status may not be set for delta results
...
def _check_period_parsing(resp: dict[str, Any]) -> tuple[bool, str]:
    """V2: Chart title or metadata should reference 2024-01 vs 2025-01."""
    ...
    # Fallback: check summary or metadata
    bc = resp.get("bank_chart", {})
    ...
    return False, (
        ...
        f"This likely means the labeled regex "
        f"'periodo inicial = enero 2024 ... periodo actual = enero 2025' "
        f"did not match."
    )


def _check_bank_coverage(resp: dict[str, Any]) -> tuple[bool, str]:
    """V3: At least 7/10 requested banks should appear in chart."""
...
def _check_invex_highlight(resp: dict[str, Any]) -> tuple[bool, str]:
    """V4: INVEX bar must be colored red (#E45756)."""
    trace = _get_first_trace(resp)
    if not trace:
    ...
    if not colors or not y_vals:
        return False, (
            "NO_COLORS: marker.color not present — "
            "Gap 2 fix may not have been applied"
        )
    ...
    return False, (
        ...
        f"highlight_bank detection may have failed."
    )


def _check_neutral_colors(resp: dict[str, Any]) -> tuple[bool, str]:
    """V5: Non-INVEX bars should be grey (#999999)."""
    trace = _get_first_trace(resp)
    if not trace:
        return False, "NO_TRACE: cannot verify colors"
...
def _check_zeroline(resp: dict[str, Any]) -> tuple[bool, str]:
    """V6: Layout.xaxis should have zeroline=true."""
    layout = _get_layout(resp)
    if not layout:
    ...
    return False, (
        f"NO_ZEROLINE: xaxis.zeroline={zeroline} — "
        f"Gap 4 fix may not have been applied"
    )


def _check_table_data(resp: dict[str, Any]) -> tuple[bool, str]:
    """V7: Response should include table_data with 4 columns."""
    bc = resp.get("bank_chart")
    if not bc:
    ...
    if not table_data:
        return False, (
            "NO_TABLE_DATA: table_data not present in response — "
            "Gap 3 fix may not have been applied"
        )
    ...
    # Check each row has 4 values and pct_change (index 3) is a number
    valid_rows = 0
    for row in rows:
...
def _check_no_fabrication(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    suspicious = re.compile(r"\$[\d,]*[05]00[,.]000[,.]000")
    raw = resp.get("content") or ""
    round_matches = suspicious.findall(raw)
    if round_matches:
        issues.append(
...
def _check_variation_values(resp: dict[str, Any]) -> tuple[bool, str]:
    """V9: Variation values in chart should be plausible percentages."""
    trace = _get_first_trace(resp)
    if not trace:
    ...
    # Variation percentages should be in plausible range (-100% to +200%)
    out_of_range = []
    none_count = 0
    ...
    return True, (
        ...
        f"range=[{min(v for v in x_vals if v is not None):.1f}%, "
        f"{max(v for v in x_vals if v is not None):.1f}%]"
    )


def _check_text_labels(resp: dict[str, Any]) -> tuple[bool, str]:
    """V10: Bar text labels should show formatted percentages."""
    trace = _get_first_trace(resp)
    if not trace:
...
def _check_no_text_contradiction(resp: dict[str, Any]) -> tuple[bool, str]:
    """V11: LLM text must NOT deny data when the chart has valid data.

    Detects the 'delta-chart-llm-text-desync' bug where the bar chart
    is correct but the LLM says "no tengo datos" because table_data
    was not injected into its context.
    """
    bc = resp.get("bank_chart")
    content = (resp.get("content") or "").lower()

    # Only check when chart actually has data
    ...
    found = []
    for phrase in TEXT_CONTRADICTION_PHRASES:
        if phrase in content:
            found.append(phrase)
...
def _check_text_chart_coherence(resp: dict[str, Any]) -> tuple[bool, str]:
    """V12: LLM text percentages must be coherent with chart values.
    ...
    # Build chart values map: {BANK: pct_change}
    chart_map: dict[str, float] = {}
    for bank, val in zip(y_vals, x_vals):
        if val is not None:
            chart_map[str(bank).upper()] = float(val)

    # Extract percentages mentioned in text near bank names.
    # Only match within the SAME line (bullet point) to avoid cross-bank
    # false positives from the introductory bank list.
    mismatches = []
    matched = 0
    lines = content.split("\n")
    for bank, chart_val in chart_map.items():
        # Look for "- BANK: ... NN.N%" on a single line
        pattern = re.compile(
        ...
        for line in lines:
            ...
            if m:
                ...
                if diff > 2.0:  # tolerance: 2 percentage points
                    mismatches.append(
...
def _check_table_bank_coverage(resp: dict[str, Any]) -> tuple[bool, str]:
    """V13: table_data rows should cover most of the 10 requested banks.

    Validates the end-to-end data flow: SQL query returned results for
...
def _check_markdown_table_in_response(resp: dict[str, Any]) -> tuple[bool, str]:
    """V14: Response text must contain a markdown table with bank data.
    ...
    When the LLM generates a partial table and the post-processor appends
    the full table_data table, both appear in the SSE-collected content.
    We check the LARGEST table found (by row count) to handle this case.
    """
    content = resp.get("content") or ""
    if not content:
    ...
    # Find ALL markdown tables and pick the largest one
    tables: list[tuple[int, int, list[str]]] = []  # (start, data_rows, banks)
    i = 0
    while i < len(lines):
        ...
        if (
            ...
            if re.match(r"^\|[\s\-:|]+(\|[\s\-:|]+)+\|$", sep):
                ...
                banks = [b for b in REQUESTED_BANKS if b in table_text]
                tables.append((table_start, data_rows, banks))
                i = j
                continue
        i += 1

    # Also accept table_data from bank_chart event (rendered in "Datos" tab)
    bc = resp.get("bank_chart") or {}
    td = bc.get("table_data") or {}
    td_rows = td.get("rows", [])
    if td_rows and len(td_rows) >= 7:
        td_banks = []
        for row in td_rows:
            if row and isinstance(row[0], str):
                td_banks.append(row[0].upper())
        matched = [b for b in REQUESTED_BANKS if b in td_banks]
        if len(matched) >= 7:
            source = "table_data"
            if tables:
                source = f"table_data (text has {len(tables)} partial table(s))"
            return True, (
                f"Table via {source}: {len(td_rows)} rows, "
                f"{len(matched)}/{len(REQUESTED_BANKS)} banks"
            )

    if not tables:
        return False, (
            "NO_TABLE: response text does not contain a markdown table "
            "and bank_chart.table_data is missing or incomplete."
        )

    # Fallback: use the largest markdown table in text
    best = max(tables, key=lambda t: t[1])
    _, data_rows, banks_in_table = best
    ...
    return True, (
        ...
        f"{' (best of ' + str(len(tables)) + ' tables)' if len(tables) > 1 else ''}"
    )
...
def run_checks(resp: dict[str, Any]) -> list[CheckResult]:
    """Run all component checks on a single response."""
    results = []
...
def main() -> int:
    print("=" * 70)
    print("E2E Test — Variacion de Cartera Total: Bar Chart Pipeline")
    ...
    if bc:
        ...
        if traces:
            trace = traces[0]
            print(f"  Type: {trace.get('type')}, orientation: {trace.get('orientation')}")
            y_vals = trace.get("y", [])
    ...
    # ── Save results JSON ──
    out = Path(__file__).parent / "variacion_cartera_results.json"
    out.write_text(
        json.dumps(
            {
                "test": "variacion-cartera-bar-chart",
                "prompt": PROMPT,
                "backend_url": BACKEND_URL,
                "total_checks": total,
                "passed": passed,
