#!/usr/bin/env python3
"""
E2E Test — Variacion de Cartera Comercial SIN GOB: Bar Chart Pipeline

Sends the exact multi-bank cartera comercial SIN ENTIDADES GUBERNAMENTALES
variation prompt and validates every component of the response: chart type,
period parsing, bank coverage, coloring (INVEX red, rest grey), zeroline,
table_data, metric detection (cartera_comercial_sin_gob), text/chart
coherence, markdown table presence, and anti-fabrication guards.

This test validates the pipeline that feeds through:
  evolucion_banco_handler._SIN_GOB_RE       → regex match
  evolucion_banco_handler._detect_metric()  → "cartera_comercial_sin_gob"
  BaseHandler._parse_period_comparison()    → ("2024-01-01", "2025-01-01")
  EvolutionUseCase.execute_delta()          → DeltaResult (from KPI table)
  DeltaResult.to_response_dict()            → horizontal bar chart

Prompt under test:
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025.
     Compara la cartera comercial sin considerar la cartera de entidades
     gubernamentales entre el periodo inicial y el periodo final entre los
     bancos: MONEX, INVEX, BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME,
     BANSI, VE POR MÁS Y BANCO BASE. ..."

Usage:
    python tests/e2e/charts/test_variacion_cc_sin_gob_bar_chart.py
"""
...
# The exact prompt — "cartera comercial sin considerar entidades gubernamentales"
PROMPT = (
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025. "
    "Compara la cartera comercial sin considerar la cartera de entidades "
    "gubernamentales entre el periodo inicial y el periodo final entre los "
...
def _check_metric_detection(resp: dict[str, Any]) -> tuple[bool, str]:
    """V3: Metric must be cartera_comercial_sin_gob.

    The prompt says "cartera comercial sin considerar la cartera de entidades
    gubernamentales", so the handler's _SIN_GOB_RE regex must fire and
    _detect_metric() should return "cartera_comercial_sin_gob".
    """
    bc = resp.get("bank_chart")
    if not bc:
    ...
    # Positive: "sin_gob" or "sin gob" should appear in title, summary, or metric_name
    sin_gob_indicators = ["sin_gob", "sin gob"]
    has_sin_gob = any(
        ind in title_lower or ind in summary_lower or ind in metric_lower
        for ind in sin_gob_indicators
    )

    if has_sin_gob:
        return True, (
            f"Metric detection OK: 'sin_gob' found. "
            f"metric_name='{metric_name}', title='{title[:80]}'"
        )

    # Negative: if it matched cartera_comercial_total instead, that's a bug
    has_comercial_total = (
        "comercial_total" in title_lower
        or "comercial_total" in summary_lower
        or "comercial_total" in metric_lower
    )
    if has_comercial_total:
        return False, (
            f"WRONG_METRIC: detected 'cartera_comercial_total' instead of "
            f"'cartera_comercial_sin_gob'. _SIN_GOB_RE regex did not match. "
            f"metric_name='{metric_name}'"
        )

    # Negative: if it matched cartera_total, even worse
    has_cartera_total = "cartera_total" in metric_lower
    if has_cartera_total:
        return False, (
            f"WRONG_METRIC: detected 'cartera_total' instead of "
            f"'cartera_comercial_sin_gob'. metric_name='{metric_name}'"
        )

    # Soft pass if "comercial" is at least present
    has_comercial = "comercial" in title_lower or "comercial" in metric_lower
    if has_comercial:
        return True, (
            f"METRIC_PARTIAL: 'comercial' found but no explicit 'sin_gob'. "
            f"metric_name='{metric_name}', title='{title[:60]}'"
        )

    return False, (
        f"METRIC_MISS: no 'sin_gob' or 'comercial' in metric_name/title. "
        f"metric_name='{metric_name}', title='{title[:60]}'"
    )
...
ALL_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck(
        ...
        description=(
            "_detect_metric() must classify 'comercial sin entidades "
            "gubernamentales' as cartera_comercial_sin_gob"
        ),
...
def main() -> int:
    print("=" * 70)
    print("E2E Test — Variacion de Cartera Comercial SIN GOB: Bar Chart Pipeline")
    ...
    # ── Save results JSON ──
    out = Path(__file__).parent / "variacion_cc_sin_gob_results.json"
    out.write_text(
        json.dumps(
            {
                "test": "variacion-cc-sin-gob-bar-chart",
                "prompt": PROMPT,
                "backend_url": BACKEND_URL,
                "total_checks": total,
                "passed": passed,
