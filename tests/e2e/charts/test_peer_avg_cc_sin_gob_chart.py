#!/usr/bin/env python3
"""
E2E Test — Peer Average Cartera Comercial SIN GOB: INVEX vs PROMEDIO
...
with INVEX and PROMEDIO series for CARTERA COMERCIAL SIN
ENTIDADES GUBERNAMENTALES.

This test complements test_peer_avg_cartera_comercial_chart.py by using
the "sin gobierno" metric variant, which excludes government entities
from the commercial portfolio (clave_tipo_cartera 1 and 2).

Prompts under test:
    A) "...cartera sin considerar ... gubernamentales de INVEX contra ... bancos: MONEX, ... De enero 2021..."
    B) "...cartera sin considerar ... gubernamentales de INVEX contra ... bancos (MONEX, ...) de enero 2021..."
    C) "...de enero 2021... cartera sin considerar ... gubernamentales de INVEX contra ... bancos: MONEX, ..."

Usage:
    python tests/e2e/charts/test_peer_avg_cc_sin_gob_chart.py
"""
...
PROMPT_A_COLON_END = (
    "Crea una gráfica donde se compare la cartera sin considerar la cartera "
    "de entidades gubernamentales de INVEX contra la cartera promedio de los "
    "bancos: MONEX, BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME, BANSI, "
...
PROMPT_C_COLON_START = (
    ...
    "donde se compare la cartera sin considerar la cartera de entidades "
    "gubernamentales de INVEX contra la cartera promedio de los bancos: "
...
def _check_values_plausible(resp: dict[str, Any]) -> tuple[bool, str]:
    """V6: Y-values should be in MDP scale for cartera sin gob.

    Cartera comercial sin gob in MDP: ~1,000 to ~500,000 MDP.
    If values exceed 1e9, the handler is returning raw pesos instead of MDP
    (missing /1e6 conversion).
    """
    traces = _get_traces(resp)
    if not traces:
        return False, "NO_TRACES"
...
def _check_layout_title(resp: dict[str, Any]) -> tuple[bool, str]:
    """V10: Chart title should reference 'sin gob' or 'sin gobierno'."""
    ...
    has_sin_gob = "sin_gob" in title_lower or "sin gob" in title_lower
    ...
    if has_comercial:
        return False, (
            f"WRONG_METRIC: '{title}' — has 'comercial' but missing 'sin_gob'. "
            f"Query may have routed to cartera_comercial_total instead of sin_gob"
        )

    return False, f"WRONG_TITLE: '{title}' — expected 'cartera_comercial_sin_gob'"
...
ALL_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck(
        ...
        description="Chart title should reference 'sin_gob' or 'sin gobierno'",
...
def main() -> int:
    ...
    print("=" * 70)
    print("E2E Test — Peer Average CC Sin Gob: INVEX vs PROMEDIO")
    ...
    # ── Save results JSON ──
    out = Path(__file__).parent / "peer_avg_cc_sin_gob_results.json"
    prompt_data = {}
    ...
    out.write_text(
        json.dumps(
            {
                "test": "peer-avg-cc-sin-gob-chart",
                "backend_url": BACKEND_URL,
                "total_checks": total,
                "passed": passed,
                "failed": failed,
