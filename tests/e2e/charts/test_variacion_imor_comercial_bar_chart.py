#!/usr/bin/env python3
"""
E2E Test — Variacion de IMOR Comercial: Bar Chart Pipeline

Sends the exact multi-bank IMOR Comercial variation prompt and validates every
component of the response: chart type, period parsing, bank coverage, coloring
(INVEX red, rest grey), zeroline, table_data, metric detection (imor_comercial),
text/chart coherence, and anti-fabrication guards.

This test validates the pipeline for IMOR Comercial variation between two periods:
  QueryRouter → handler match → _parse_period_comparison()
  → delta or time-series path → horizontal bar chart

IMOR Comercial = (E3_SG + Castigos) / (E1_SG + E2_SG + E3_SG)
where E*_SG = Act.Empresarial + Ent.Financieras (Sin Gobierno)
This is the IMORA formula from Tableau TWB (Invex_Tablero_V3.twb).

The metric maps to hip_imor_comercial in _METRIC_MAP via:
  "imor comercial" → "hip_imor_comercial"

Prompt under test:
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025.
     Compara el IMOR Comercial entre el periodo inicial y el periodo final
     entre los bancos: MONEX, INVEX, BANCREA, SABADELL, BANCA MIFEL,
     ...
     Banco | IMOR Comercial 2024 | IMOR Comercial 2025 | % Variacion"

Usage:
    python tests/e2e/charts/test_variacion_imor_comercial_bar_chart.py
"""
...
# The exact prompt — note "IMOR Comercial" (routes to hip_imor_comercial)
PROMPT = (
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025. "
    "Compara el IMOR Comercial entre el periodo inicial y el periodo final entre los "
...
def _check_metric_detection(resp: dict[str, Any]) -> tuple[bool, str]:
    """V3: Metric must be IMOR Comercial (not cartera_total, not ICOR).

    The prompt says "IMOR Comercial" explicitly, so the response should
    reference IMOR / morosidad / comercial somewhere.
    """
    bc = resp.get("bank_chart")
    if not bc:
    ...
    # Positive: "imor" or "comercial" or "morosidad" should appear
    has_imor = (
        "imor" in title_lower
        ...
        or "imor" in content
        or "comercial" in title_lower
        or "vencida comercial" in content
    )

    # Negative: should NOT say "cartera_total" as the primary metric
    has_wrong_metric = (
        "cartera_total" in title_lower
        or "cartera total" in title_lower
    )

    if has_imor:
        return True, (
            f"Metric detection OK: IMOR/comercial/morosidad found. "
            f"title='{title}', summary='{summary[:60]}'"
        )

    if has_wrong_metric:
        return False, (
            f"WRONG_METRIC: detected cartera total instead of IMOR Comercial. "
            f"title='{title}'"
        )

    return True, (
        f"METRIC_INDIRECT: no explicit 'imor' in title/summary, "
        f"but query explicitly asks for IMOR Comercial. title='{title[:50]}'"
    )
...
def _check_table_data(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    Expected: Banco | IMOR Comercial 2024 | IMOR Comercial 2025 | % Variacion
    """
    bc = resp.get("bank_chart")
    if not bc:
...
def _check_variation_values(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    IMOR Comercial values are typically 0.5%-10% for Mexican banks.
    Variation between periods can be extreme (bank going from 0.5% to 2% is
    +300% variation). We allow: -100% to +5000%.
    """
    trace = _get_first_trace(resp)
    if not trace:
...
def _check_text_chart_coherence(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    For IMOR Comercial, the LLM often writes absolute metric values
    (e.g. "INVEX: 2.35%") while the chart shows variation percentages
    (e.g. "+14.2%"). These are categorically different scales, so this
    validator treats that as a soft pass.
    """
    bc = resp.get("bank_chart")
    ...
    if text_vals_found and matched == 0:
        ...
        if avg_text < 20 and avg_chart > 20:
            return True, (
                f"SOFT_PASS: LLM reports absolute IMOR Comercial values "
...
ALL_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck(
        ...
        description=(
            "Response must reference IMOR Comercial / morosidad "
            "(not cartera_total)"
        ),
    ...
    ComponentCheck(
        ...
        description=(
            "IMOR Comercial variation percentages must be in plausible range "
            "[-100%, +5000%]"
        ),
...
def main() -> int:
    print("=" * 70)
    print("E2E Test — Variacion de IMOR Comercial: Bar Chart Pipeline")
    ...
    # -- Save results JSON --
    out = Path(__file__).parent / "variacion_imor_comercial_results.json"
    out.write_text(
        json.dumps(
            {
                "test": "variacion-imor-comercial-bar-chart",
                "prompt": PROMPT,
                "backend_url": BACKEND_URL,
                "total_checks": total,
                "passed": passed,
