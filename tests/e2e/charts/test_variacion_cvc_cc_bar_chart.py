#!/usr/bin/env python3
"""
E2E Test — Variación CVC/CC (Cartera Vencida Comercial / Cartera Comercial): Bar Chart Pipeline

Sends the exact multi-bank CVC/CC ratio variation prompt and validates
...
(ratio vencida/comercial), text/chart coherence, and anti-fabrication guards.

This test validates the pipeline that feeds through:
  evolucion_banco_handler._detect_metric()  → ratio metric (vencida/comercial)
...
Prompt under test:
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025.
     Compara la razón de cartera vencida comercial entre la cartera comercial
     para el periodo inicial y el periodo final entre los bancos: MONEX, INVEX,
     BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME, BANSI, VE POR MÁS Y
     BANCO BASE. Presenta el dato del periodo inicial, el dato del periodo final
     y la variación entre el periodo inicial. Donde la variación es =
     (periodo actual / periodo inicial -1). Haz una gráfica de barras donde se
     vea la variación graficada y marca a INVEX de color rojo. Así como una
     tabla con: Banco | CVC/CC 2024 | CVC/CC 2025 | % Variación"

Usage:
    python tests/e2e/charts/test_variacion_cvc_cc_bar_chart.py
"""
...
# The exact prompt — ratio CVC/CC (cartera vencida comercial / cartera comercial)
PROMPT = (
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025. "
    "Compara la razón de cartera vencida comercial entre la cartera comercial "
    "para el periodo inicial y el periodo final entre los bancos: "
    "MONEX, INVEX, BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME, BANSI, "
    "VE POR MÁS Y BANCO BASE. "
    "Presenta el dato del periodo inicial, el dato del periodo final y la "
    "variación entre el periodo inicial. "
    ...
    "Banco | CVC/CC 2024 | CVC/CC 2025 | % Variación"
)
...
def _check_metric_detection(resp: dict[str, Any]) -> tuple[bool, str]:
    """V3: Metric must be a ratio involving cartera vencida.

    The prompt says "razón de cartera vencida comercial entre la cartera
    comercial", so the handler should detect a ratio metric (vencida/comercial),
    NOT plain cartera_comercial_total or cartera_total.
    """
    bc = resp.get("bank_chart")
    if not bc:
    ...
    # Positive: "vencida" should appear somewhere (it's the distinguishing term)
    has_vencida = "vencida" in combined
    # Also accept ratio-related terms or "imor comercial" (= CVC/CC)
    has_ratio = any(kw in combined for kw in (
        "razón", "razon", "cvc", "ratio", "imor comercial", "imor_comercial",
    ))
    # Negative: should NOT be plain cartera_comercial or cartera_total without vencida
    has_plain_metric = (
        ("cartera_comercial" in combined or "cartera comercial" in combined)
        and not has_vencida
        and not has_ratio
    )

    if has_vencida or has_ratio:
        return True, (
            f"Metric detection OK: ratio metric detected. "
            f"title='{title}', summary='{summary[:60]}'"
        )

    if has_plain_metric:
        return False, (
            f"WRONG_METRIC: detected plain 'cartera_comercial' instead of "
            f"ratio vencida/comercial. title='{title}'"
        )

    # If neither is found, check metadata
    metric_type = metadata.get("metric_type", "")
    return True, (
        f"METRIC_INDIRECT: no explicit 'vencida' in title/summary, "
...
def _check_variation_values(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    For a ratio metric (CVC/CC), variations can be larger than for absolute
    metrics because small base values amplify percentage changes.
    We use a wider range: [-100%, +500%].
    """
    trace = _get_first_trace(resp)
    if not trace:
    ...
    if out_of_range:
        return False, (
            f"IMPLAUSIBLE: {len(out_of_range)} values out of [-100, 500] range: "
            f"{out_of_range[:5]}"
        )
...
def _check_text_chart_coherence(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    For ratio metrics like CVC/CC, both the base values AND the variation
    are percentages (e.g. "1.6% → 2.4% (+49.2%)").  We use a greedy
    quantifier so the regex skips past earlier percentages and captures
    the LAST one on the line — which is the variation.
    """
    bc = resp.get("bank_chart")
    ...
    if not traces:
        return False, "FAIL: plotly_config.data is empty (0 traces)"
    trace = traces[0]
    x_vals = trace.get("x", [])
    y_vals = trace.get("y", [])
    ...
    for bank, chart_val in chart_map.items():
        # Anchor on "variación" / "variacion" keyword that always precedes
        # the variation percentage, skipping past base-value percentages.
        pattern = re.compile(
            rf"[-•]\s*{re.escape(bank)}.*?variaci[oó]n\s*\(?([+-]?\d+[.,]\d+)\s*%\)?",
            re.IGNORECASE,
        )
        for line in lines:
...
ALL_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck(
        ...
        description=(
            "_detect_metric() must classify as ratio vencida/comercial "
            "(not plain cartera_comercial_total or cartera_total)"
        ),
    ...
    ComponentCheck(
        ...
        description="Variation percentages must be in plausible range [-100, 500]",
...
def main() -> int:
    print("=" * 70)
    print("E2E Test — Variación CVC/CC (Vencida/Comercial): Bar Chart Pipeline")
    ...
    # ── Save results JSON ──
    out = Path(__file__).parent / "variacion_cvc_cc_results.json"
    out.write_text(
        json.dumps(
            {
                "test": "variacion-cvc-cc-bar-chart",
                "prompt": PROMPT,
                "backend_url": BACKEND_URL,
                "total_checks": total,
                "passed": passed,
