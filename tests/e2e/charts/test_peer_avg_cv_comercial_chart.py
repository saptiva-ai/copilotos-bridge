#!/usr/bin/env python3
"""
E2E Test — Peer Average Razón Cartera Vencida Comercial: INVEX vs PROMEDIO

Validates that a prompt requesting the ratio of overdue commercial portfolio
(cartera vencida comercial / cartera comercial) routes to the peer-average
handler and produces a comparison chart with INVEX and PROMEDIO series.

Unlike the monetary cartera comercial test, this test validates a RATIO metric
where Y-axis values are percentages (typically 0.5% – 20%) instead of absolute
peso amounts.

Prompt under test:
    "Crea una gráfica donde se compare razón de cartera vencida comercial
     entre la cartera comercial de INVEX contra la cartera promedio de los
     bancos: MONEX, BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME,
     BANSI, VE POR MÁS Y BANCO BASE. De Enero 2021 hasta el dato más
     reciente que tengas."

Usage:
    python tests/e2e/charts/test_peer_avg_cv_comercial_chart.py
"""
...
def _check_values_are_ratio(resp: dict[str, Any]) -> tuple[bool, str]:
    """V6: Y-values must be in a plausible ratio range (0–100%).

    Cartera vencida comercial / cartera comercial for Mexican banks is
    typically between 0.5% and 20%.  Values above 50% or below 0 indicate
    a wrong metric was served (e.g. monetary amounts instead of a ratio).
    """
    traces = _get_traces(resp)
    if not traces:
        return False, "NO_TRACES"
    ...
    for t in traces:
        ...
        # If values are in the thousands/millions, the metric is monetary,
        # not a ratio — wrong data was served.
        if max_val > 100:
            return False, (
                f"NOT_RATIO: {name} max={max_val:.2f} — "
                f"values exceed 100%, likely monetary instead of ratio"
            )

    # Report the actual range seen
    all_vals = [
        v for t in traces for v in t.get("y", []) if v is not None
    ]
    return True, (
        f"Ratio range OK: min={min(all_vals):.2f}%, max={max(all_vals):.2f}%"
    )


def _check_promedio_is_ratio(resp: dict[str, Any]) -> tuple[bool, str]:
    """V7: PROMEDIO should be a reasonable peer-average ratio, not a sum.

    For a ratio metric, the average should be in a similar range as
    individual banks (0–50%).  If it exceeds 50%, it may be a sum of
    ratios rather than an average.
    """
    traces = _get_traces(resp)
    promedio_trace = None
    for t in traces:
    ...
    return True, (
        f"Promedio ratio OK: avg={avg_val:.2f}%, max={max_val:.2f}%"
    )
...
def _check_layout_title(resp: dict[str, Any]) -> tuple[bool, str]:
    """V10: Chart title should reference 'vencida' or 'cartera vencida'."""
    ...
    has_vencida = "vencida" in title_lower
    has_comercial = "comercial" in title_lower

    if has_vencida:
        return True, f"Title OK: '{title}'"

    # Partial match — has 'comercial' but not 'vencida'
    if has_comercial:
        return False, (
            f"PARTIAL_TITLE: '{title}' — has 'comercial' but missing "
            f"'vencida' (may be showing cartera total instead of ratio)"
        )

    has_cartera = "cartera" in title_lower
    if has_cartera:
        return False, (
            f"PARTIAL_TITLE: '{title}' — has 'cartera' but missing 'vencida'"
        )

    return False, f"WRONG_TITLE: '{title}' — expected 'cartera vencida'"
...
def _parse_percentage_values(text: str) -> list[tuple[float, str]]:
    ...
    Returns list of (percentage_value, original_match_text).
    """
    results: list[tuple[float, str]] = []

    # Pattern 1: "N.N%" or "N.N %" (with optional space)
    pct_pat = re.compile(
        r"(\d{1,3}(?:[.,]\d{1,4})?)\s*%",
    )
    ...
    porciento_pat = re.compile(
        r"(\d{1,3}(?:[.,]\d{1,4})?)\s+por\s+ciento",
        re.IGNORECASE,
    )
    for m in porciento_pat.finditer(text):
        raw = m.group(1).replace(",", ".")
        try:
            val = float(raw)
            if 0 < val < 100:
...
def _check_text_values_coherence(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    Tolerance: 30% relative difference (wider than monetary because ratios
    are smaller numbers and LLM may round aggressively).
    """
        ...
        if best_match:
            ...
            if rel_diff > 0.30:
                mismatches.append(
                    ...
                    f"chart={chart_val:.2f}% (diff={rel_diff:.0%})"
                )
            else:
                matched += 1
...
def _check_text_direction_coherence(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    Note: for cartera vencida, higher = worse (more overdue loans),
    but we still validate directional accuracy regardless of semantics.
    """
    content = (resp.get("content") or "").lower()
    if not content:
        return True, "SKIP: no LLM text"
...
ALL_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck(
        ...
        description="Y-values must be percentages (0–100%), not monetary amounts",
        validate=_check_values_are_ratio,
    ),
    ComponentCheck(
        name="V7_PROMEDIO_IS_RATIO",
        description="PROMEDIO should be a peer-average ratio, not a sum",
    ...
    ComponentCheck(
        ...
        description="Chart title should reference 'cartera vencida'",
    ...
    ComponentCheck(
        ...
        description="Percentage values in LLM text must match chart Y-values (within 30%)",
...
def main() -> int:
    print("=" * 70)
    print("E2E Test — Peer Average Razón CV Comercial: INVEX vs PROMEDIO")
    ...
    if bc:
        ...
        for t in traces:
            ...
            if y_vals:
                print(
                    f"    {name}: min={min(y_vals):.2f}, "
                    f"max={max(y_vals):.2f}, last={y_vals[-1]:.2f}"
                )
    ...
    # ── Save results JSON ──
    out = Path(__file__).parent / "peer_avg_cv_comercial_results.json"
    out.write_text(
        json.dumps(
            {
                "test": "peer-avg-cv-comercial-chart",
                "backend_url": BACKEND_URL,
                "total_checks": total,
                "passed": passed,
                "failed": failed,
