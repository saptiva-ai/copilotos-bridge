#!/usr/bin/env python3
"""
E2E Test — Peer Average ICAP: INVEX vs PROMEDIO
...
with INVEX and PROMEDIO series for ICAP (Índice de Capitalización).

Tableau reference view: "ICAP Comparativo" — dual Y-axis line chart
  - Red line: Invex ICAP
  - Grey line: Prom. ICAP (peer average)
  - Range: 04/2016 — 03/2025
  - Values: percentages (typically 10–25%)

Prompts under test (date always at start for reliable extraction):
    A) "de enero 2017 ... ICAP de INVEX ... bancos: MONEX, ..."
    B) "de enero 2017 ... ICAP de INVEX ... bancos (MONEX, ...)"
    C) "de enero 2017 ... índice de capitalización de INVEX ... bancos: MONEX, ..."

Usage:
    python tests/e2e/charts/test_peer_avg_icap_chart.py
"""
...
# ── Prompt variants ──────────────────────────────────────────────────────────
# All variants place the date range BEFORE the bank list — proven to produce
# correct date extraction by the LLM (date-at-end loses to lost-in-the-middle).
#
# Variant A: onboarding format — colon after "bancos".
PROMPT_A_COLON = (
    "Crea una gráfica de enero 2017 hasta el dato más reciente que tengas "
    "donde se compare el ICAP de INVEX contra el promedio de los bancos: "
    "MONEX, BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME, BANSI, "
    "VE POR MAS Y BANCO BASE"
)
# Variant B: parenthesized bank list.
PROMPT_B_PARENS = (
    "Crea una gráfica de enero 2017 hasta el dato más reciente que tengas "
    "donde se compare el ICAP de INVEX contra el promedio de los bancos "
    "(MONEX, BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME, BANSI, "
...
# Variant C: explicit "índice de capitalización" instead of "ICAP".
PROMPT_C_LONG_NAME = (
    "Crea una gráfica de enero 2017 hasta el dato más reciente que tengas "
    "donde se compare el índice de capitalización de INVEX contra el promedio "
    "de los bancos: MONEX, BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME, "
    "BANSI, VE POR MAS Y BANCO BASE"
)

ALL_PROMPTS = [
    ("A (colon)", PROMPT_A_COLON),
    ("B (parens)", PROMPT_B_PARENS),
    ("C (long name)", PROMPT_C_LONG_NAME),
]
...
def _check_period_coverage(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    has_2017 = any("2017" in d for d in all_dates)
    has_recent = any(d >= "2024" for d in all_dates)

    if has_2017 and has_recent:
        return True, (
...
def _check_data_point_count(resp: dict[str, Any]) -> tuple[bool, str]:
    """V5: Each series should have >= 90 data points (~8 years monthly from Jan 2017)."""
    traces = _get_traces(resp)
    if not traces:
    ...
    for t in traces:
        ...
        if len(y_vals) < 90:
            issues.append(f"{name}: {len(y_vals)} points (expected >=90)")
...
def _check_values_plausible(resp: dict[str, Any]) -> tuple[bool, str]:
    """V6: ICAP percentages should be between 5 and 40 for these banks."""
    traces = _get_traces(resp)
    if not traces:
        return False, "NO_TRACES"
    ...
    for t in traces:
        ...
        if min_val < 5 or max_val > 40:
            return False, (
                f"OUT_OF_RANGE: {name} min={min_val:.2f}, max={max_val:.2f} "
                f"(expected 5–40 for ICAP %)"
            )

    return True, "Values plausible: all ICAP percentages in [5, 40] range"


def _check_promedio_plausible(resp: dict[str, Any]) -> tuple[bool, str]:
    """V7: PROMEDIO should be a peer average in plausible ICAP range."""
    traces = _get_traces(resp)
    promedio_trace = None
    for t in traces:
    ...
    if max_val > 40 or min_val < 5:
        return False, (
            f"IMPLAUSIBLE_AVG: PROMEDIO range [{min_val:.2f}, {max_val:.2f}] — "
            f"expected [5, 40] for ICAP peer average"
        )

    return True, (
        f"Promedio magnitude OK: avg={avg_val:.2f}%, "
...
def _check_layout_title(resp: dict[str, Any]) -> tuple[bool, str]:
    """V10: Chart title should reference ICAP or capitalización."""
    ...
    if "icap" in title_lower or "capitalizacion" in title_lower or "capitalización" in title_lower:
        return True, f"Title OK: '{title}'"

    return False, f"WRONG_TITLE: '{title}' — expected 'ICAP' or 'capitalización'"
...
def _check_text_values_coherence(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    and compares against any percentage values the LLM mentions in the text.
    Tolerance: 2 percentage points absolute difference.
    """
    ...
    for trace_name, chart_val in chart_vals.items():
            ...
            for alias in search_aliases:
                ...
                while pos != -1:
                    ...
                    if text_pos != -1 and abs(text_pos - pos) < 300:
                        abs_diff = abs(text_val - chart_val)
                        if abs_diff < best_distance:
                            best_distance = abs_diff
        ...
        if best_match:
            ...
            abs_diff = abs(text_val - chart_val)
            if abs_diff > 2.0:  # 2 pp tolerance
                mismatches.append(
                    f"{trace_name}: text='{text_str}' ({text_val:.2f}%) vs "
                    f"chart={chart_val:.2f}% (diff={abs_diff:.2f} pp)"
                )
            else:
                matched += 1
    ...
    if matched > 0:
        return True, (
            ...
            f"match chart data (within 2 pp)"
        )
...
def _check_text_direction_coherence(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    When the difference is < 1 pp, directional ambiguity is expected
    (the LLM may describe historical periods where direction differed),
    so a soft pass is given even if phrases conflict.
    """
    content = (resp.get("content") or "").lower()
    if not content:
        return True, "SKIP: no LLM text"
    ...
    abs_diff = abs(invex_val - promedio_val)
    ...
    if wrong_claims:
        ...
        # With < 1 pp difference, directional ambiguity is expected —
        # the LLM may describe historical periods where direction differed.
        if abs_diff < 1.0:
            return True, (
                f"SOFT_PASS: INVEX is {direction} PROMEDIO by only {abs_diff:.2f} pp "
                f"(INVEX={invex_val:.2f}%, PROMEDIO={promedio_val:.2f}%) — "
                f"direction claim '{wrong_claims[0]}' tolerated due to small gap"
            )
        return False, (
            f"DIRECTION_WRONG: INVEX is {direction} PROMEDIO "
            f"(INVEX={invex_val:.2f}%, PROMEDIO={promedio_val:.2f}%, "
            f"diff={abs_diff:.2f} pp) "
            f"but text claims opposite: "
            + " | ".join(wrong_claims[:3])
        )
...
ALL_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck(
        name="V5_DATA_POINTS",
        description="Each series should have >= 90 monthly data points (~8 years)",
        validate=_check_data_point_count,
    ),
    ComponentCheck(
        name="V6_VALUES_PLAUSIBLE",
        description="ICAP percentages must be in [5, 40] range",
        validate=_check_values_plausible,
    ),
    ComponentCheck(
        name="V7_PROMEDIO_MAGNITUDE",
        description="PROMEDIO should be a plausible ICAP peer average (5–40%)",
    ...
    ComponentCheck(
        ...
        description="Chart title should reference 'ICAP' or 'capitalización'",
    ...
    ComponentCheck(
        ...
        description="Percentage values in LLM text must match chart Y-values (within 2 pp)",
...
def main() -> int:
    ...
    print("=" * 70)
    print("E2E Test — Peer Average ICAP: INVEX vs PROMEDIO")
    ...
    # ── Save results JSON ──
    out = Path(__file__).parent / "peer_avg_icap_results.json"
    prompt_data = {}
