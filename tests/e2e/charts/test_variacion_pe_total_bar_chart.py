#!/usr/bin/env python3
"""
E2E Test — Variacion de Pérdida Esperada Total: Bar Chart Pipeline

Sends a multi-bank PE (Pérdida Esperada) variation prompt and validates
...
(pe_total), text/chart coherence, and anti-fabrication guards.

PE Total = Reservas / Cartera Total (ratio metric, values ~0.5%–10%).

This test validates the pipeline that feeds through:
  QuerySpecParser                          → metric="PE_TOTAL"
  metricas_financieras or evolucion_banco  → handler routing
  execute_delta() or NL2SQL               → DeltaResult / bar chart
  DeltaResult.to_response_dict()          → horizontal bar chart

Prompt under test:
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025.
     Perdida esperada total es calculada como las reservas entre la cartera
     total. Presenta el dato del periodo inicial, el dato del periodo final
     y la variación entre el periodo inicial y periodo final entre los bancos:
     MONEX, INVEX, BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME, BANSI,
     VE POR MÁS Y BANCO BASE. Done la variacion es = (periodo actual /
     periodo inicial -1). Haz una gráfica de barras donde se vea la variacion
     graficada y marca a invex de color rojo. Así como una tabla con:
     Banco | PE Total 2024 | PE Total 2025 | % Variación"

Usage:
    python tests/e2e/charts/test_variacion_pe_total_bar_chart.py
"""
...
# The exact prompt — note "Perdida esperada total" (ratio metric)
PROMPT = (
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025. "
    "Perdida esperada total es calculada como las reservas entre la cartera total. "
    "Presenta el dato del periodo inicial, el dato del periodo final y la variación "
    "entre el periodo inicial y periodo final entre los bancos: MONEX, INVEX, "
...
def _check_metric_detection(resp: dict[str, Any]) -> tuple[bool, str]:
    """V3: Metric must be PE_TOTAL (not cartera_total or cartera_comercial).

    The prompt says "Perdida esperada total" so the handler should detect
    PE_TOTAL, NOT cartera_total.
    """
    bc = resp.get("bank_chart")
    if not bc:
    ...
    summary = bc.get("summary", "")
    metric_name = bc.get("metric_name", "")

    title_lower = title.lower()
    summary_lower = summary.lower()
    metric_lower = metric_name.lower()

    # PE indicators: "pe", "pe_total", "perdida", "pérdida", "esperada"
    pe_indicators = ["pe_total", "pe total", "perdida esperada", "pérdida esperada", "pe "]
    has_pe = any(
        ind in title_lower or ind in summary_lower or ind in metric_lower
        for ind in pe_indicators
    )

    if has_pe:
        return True, (
            f"Metric detection OK: PE found. "
            f"metric_name='{metric_name}', title='{title[:80]}'"
        )

    # Negative: should NOT say "cartera_total" or "cartera_comercial"
    wrong_cartera = any(
        kw in title_lower or kw in metric_lower
        for kw in ["cartera_total", "cartera_comercial", "comercial_total"]
    )

    if wrong_cartera:
        return False, (
            f"WRONG_METRIC: detected cartera instead of PE. "
            f"metric_name='{metric_name}', title='{title[:80]}'. "
            f"PE_TOTAL may not be in _METRIC_MAP or handler routing failed."
        )

    return False, (
        f"PE_NOT_FOUND: no PE indicator in title/summary/metric_name. "
        f"metric_name='{metric_name}', title='{title[:60]}', summary='{summary[:60]}'"
    )
...
def _check_table_data(resp: dict[str, Any]) -> tuple[bool, str]:
    """V8: Response should include table_data with at least 3 columns."""
    bc = resp.get("bank_chart")
    if not bc:
    ...
    if len(columns) < 3:
        return False, f"WRONG_COLUMNS: expected >=3, got {len(columns)}: {columns}"

    if not rows:
        return False, "EMPTY_TABLE: table_data.rows is empty"

    has_banco = any("banco" in c.lower() for c in columns)
    has_variacion = any("variaci" in c.lower() or "cambio" in c.lower() for c in columns)

    if not has_banco:
        return False, f"BAD_COLUMNS: columns={columns}, missing Banco"

    # Check each row has values and the last column (pct_change) is a number
    valid_rows = 0
    for row in rows:
...
def _check_variation_values(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    for i, val in enumerate(x_vals):
        ...
        try:
            v = float(val)
            # PE variation can be volatile, allow wider range
            if v < -100 or v > 500:
...
def _check_text_chart_coherence(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    for bank, chart_val in chart_map.items():
        # For ratio metrics (PE), text has "0.67% → 0.94% (+41.07%)"
        # Must match the VARIATION (signed, in parens), not the raw values.
        # Parenthesized signed percentage: (+41.07%) or (-18.55%)
        paren_pattern = re.compile(
            rf"[-•]\s*{re.escape(bank)}.*\(([+-]\d+[.,]\d+)\s*%\)",
            re.IGNORECASE,
        )
        # Fallback: last signed percentage on line (no parens required)
        sign_pattern = re.compile(
            rf"[-•]\s*{re.escape(bank)}.*([+-]\d+[.,]\d+)\s*%",
            re.IGNORECASE,
        )
        for line in lines:
            m = paren_pattern.search(line) or sign_pattern.search(line)
            if m:
...
ALL_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck(
        ...
        description="Period must reference 2024 vs 2025",
        validate=_check_period_parsing,
    ),
    ComponentCheck(
        name="V3_METRIC_DETECTION",
        description=(
            "Metric must be PE_TOTAL "
    ...
    ComponentCheck(
        name="V8_TABLE_DATA",
        description="Response must include table_data with columns",
...
def main() -> int:
    print("=" * 70)
    print("E2E Test — Variacion de PE Total: Bar Chart Pipeline")
    ...
    # ── Save results JSON ──
    out = Path(__file__).parent / "variacion_pe_total_results.json"
    out.write_text(
        json.dumps(
            {
                "test": "variacion-pe-total-bar-chart",
                "prompt": PROMPT,
                "backend_url": BACKEND_URL,
                "total_checks": total,
                "passed": passed,
