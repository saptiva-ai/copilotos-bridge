#!/usr/bin/env python3
"""
E2E Test — Peer Average: INVEX vs PROMEDIO Line Chart

Validates that both prompt variants (colon-separated and parenthesized bank
lists) produce a correct peer-average comparison chart with INVEX and
PROMEDIO series.

Root cause: The heuristic parser used space-padded string markers to detect
"promedio de los bancos". A colon after "bancos" (Prompt A) broke the trailing
space match, causing the query to fall through to the generic evolution
handler which returned all 46 banks instead of the peer average.

Prompts under test:
    A) "...INVEX contra la cartera promedio de los bancos: MONEX, ..."
    B) "...INVEX contra la cartera promedio de los bancos (MONEX, ...)"

Usage:
    python tests/e2e/charts/test_peer_average_chart.py
"""
...
# The 9 peer banks requested in both prompts
PEER_BANKS = [
    "MONEX", "BANCREA", "SABADELL", "MIFEL",
    "MULTIVA", "AFIRME", "BANSI", "VE POR MAS", "BANCO BASE",
]

# Two prompt variants that must produce identical results
PROMPT_A_COLON = (
    "Crea una gráfica de enero 2021 hasta el dato mas reciente que tengas "
...
def _check_not_all_banks(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    The original bug showed all 46 banks when the peer-average parser failed.
    """
    trace_names = _extract_trace_names(resp)
    bc = resp.get("bank_chart", {})
    ...
    # If we see system-wide banks that weren't requested, it's a regression
...
def _check_values_plausible(resp: dict[str, Any]) -> tuple[bool, str]:
    """V6: Y-values should be in MDP scale for cartera comercial.

    Cartera comercial values should be in MDP (millions of pesos).
    Typical range: 1,000 MDP to 500,000 MDP.
    If values exceed 1e9, the handler is returning raw pesos instead of MDP
    (missing /1e6 conversion in regular KPI path).
    """
    traces = _get_traces(resp)
    if not traces:
        return False, "NO_TRACES"
    ...
    for t in traces:
        ...
        # Values should be in MDP scale, not raw pesos
        if max_val > 1_000_000_000:
            return False, (
                f"SCALE_BUG: {name} max={max_val:,.0f} — values appear to be "
                f"in raw pesos instead of MDP. Handler likely missing /1e6 "
                f"conversion in regular KPI path (peer_average.py)"
            )

    return True, "Values plausible (MDP scale) across all series"


def _check_promedio_smaller_than_system(resp: dict[str, Any]) -> tuple[bool, str]:
    """V7: PROMEDIO should be smaller than SISTEMA total.

    A peer average of ~9 mid-size banks should be much smaller than the
    system total. If PROMEDIO > 500,000 MDP, something is wrong.
    """
    traces = _get_traces(resp)
    promedio_trace = None
    for t in traces:
    ...
    # In MDP: peer average ~30,000-50,000 MDP is typical
    # In raw pesos: ~3e10 - 5e10
    # System total would be ~3,500,000 MDP or ~3.5e12
    ...
    return True, (
        f"Promedio magnitude OK: max={max_val:.2e}"
    )
...
def _parse_monetary_values(text: str) -> list[tuple[float, str]]:
    """Extract monetary values from LLM text, in MDP scale.

    Chart Y-values are in MDP (millions of pesos), so text values must
    also be normalized to MDP for comparison.

    Handles patterns like:
      - "$14,230 millones" → 14,230 MDP
      - "15,665 MDP" → 15,665 MDP
      - "16,402,586,992" → 16,403 MDP (raw pesos / 1e6)

    Returns list of (value_in_mdp, original_match_text).
    """
    results: list[tuple[float, str]] = []

    # Pattern 1: "$NN,NNN millones" or "NN,NNN MDP" or "NN,NNN millones de pesos"
    # These are already in MDP scale — no multiplication needed.
    mdp_pat = re.compile(
    ...
    for m in raw_pat.finditer(text):
        ...
        try:
            val = float(raw)
            if val >= 1e9:
                results.append((val / 1e6, m.group(0)))  # pesos → MDP
        except ValueError:
            pass
...
def main() -> int:
    print("=" * 70)
    print("E2E Test — Peer Average: INVEX vs PROMEDIO")
    print(f"Backend: {BACKEND_URL}")
    print(f"Checks: {len(ALL_CHECKS)} component validators x 2 prompts")
    print("=" * 70)
    ...
    # ── Run both prompts ──
    results_a, resp_a = run_single_prompt(token, PROMPT_A_COLON, "A (colon)")
    results_b, resp_b = run_single_prompt(token, PROMPT_B_PARENS, "B (parens)")
    ...
    names_a = set(_extract_trace_names({"bank_chart": resp_a.get("bank_chart")}))
    names_b = set(_extract_trace_names({"bank_chart": resp_b.get("bank_chart")}))

    if names_a and names_b and names_a == names_b:
        print(f"  [PASS] Both prompts produced same series: {sorted(names_a)}")
        consistency_ok = True
    elif names_a and names_b:
        diff = names_a.symmetric_difference(names_b)
        print(f"  [FAIL] Series mismatch: A={sorted(names_a)}, B={sorted(names_b)}")
        print(f"         Difference: {sorted(diff)}")
        consistency_ok = False
    else:
        print("  [SKIP] Cannot compare — one or both prompts had no traces")
        consistency_ok = True
    ...
    print(
        f"  Prompt A: {sum(1 for r in results_a if r.passed)}/{len(results_a)}"
    )
    print(
        f"  Prompt B: {sum(1 for r in results_b if r.passed)}/{len(results_b)}"
    )
    print(
    ...
    # ── Save results JSON ──
    out = Path(__file__).parent / "peer_average_results.json"
    out.write_text(
        json.dumps(
            {
                "test": "peer-average-chart",
                "backend_url": BACKEND_URL,
                "total_checks": total,
                "passed": passed,
                "failed": failed,
                ...
                "prompt_a": {
                    ...
                    "response_summary": {
                        "content_length": len(resp_a.get("content", "")),
                        "has_chart": resp_a.get("bank_chart") is not None,
                        "trace_names": list(names_a),
                    },
                },
                "prompt_b": {
                    "prompt": PROMPT_B_PARENS,
                    "checks": [
                        {
                            "name": r.check.name,
                            "passed": r.passed,
                    ...
                    "response_summary": {
                        "content_length": len(resp_b.get("content", "")),
                        "has_chart": resp_b.get("bank_chart") is not None,
                        "trace_names": list(names_b),
                    },
                },
            },
            indent=2,
