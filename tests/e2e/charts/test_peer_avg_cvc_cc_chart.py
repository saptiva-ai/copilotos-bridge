#!/usr/bin/env python3
"""
E2E Test — Peer Average CVC/CC (Cartera Vencida Comercial / Cartera Comercial)
...
with INVEX and PROMEDIO series for the CVC/CC ratio metric.

This is a RATIO metric (Y-axis = percentages, typically 0.5%–20%)
as opposed to the monetary cartera comercial test. Validators are
adapted accordingly: no monetary parsing, percentage range checks.

Prompts under test:
    A) "...razón de cartera vencida comercial entre la cartera comercial
        de INVEX contra ... bancos: MONEX, ... De octubre 2022..."
    B) "...razón de cartera vencida comercial entre la cartera comercial
        de INVEX contra ... bancos (MONEX, ...) de octubre 2022..."
    C) "...de octubre 2022... razón de cartera vencida comercial entre la
        cartera comercial de INVEX contra ... bancos: MONEX, ..."

Usage:
    python tests/e2e/charts/test_peer_avg_cvc_cc_chart.py
"""
...
PROMPT_A_COLON_END = (
    ...
    "entre la cartera comercial de INVEX contra el promedio de los "
...
PROMPT_C_COLON_START = (
    ...
    "donde se compare razón de cartera vencida comercial entre la cartera "
    "comercial de INVEX contra el promedio de los bancos: MONEX, "
...
def _check_values_are_ratio(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    CVC/CC for Mexican banks is typically between 0.5% and 20%.
    Values above 100% or below 0 indicate a wrong metric was served
    (e.g. monetary amounts instead of a ratio).
    """
    traces = _get_traces(resp)
    if not traces:
        return False, "NO_TRACES"
...
def _check_layout_title(resp: dict[str, Any]) -> tuple[bool, str]:
    """V10: Chart title should reference 'vencida' or 'CVC/CC'."""
    ...
    if "cvc" in title_lower or "vencida" in title_lower:
        return True, f"Title OK: '{title}'"

    if "comercial" in title_lower:
        return False, (
            f"PARTIAL_TITLE: '{title}' — has 'comercial' but missing "
            f"'vencida' or 'CVC' (may be showing cartera total instead)"
        )

    return False, f"WRONG_TITLE: '{title}' — expected 'vencida' or 'CVC/CC'"
...
def _check_text_direction_coherence(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    Note: for CVC/CC, higher = worse (more overdue loans),
...
ALL_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck(
        ...
        description="Chart title should reference 'vencida' or 'CVC/CC'",
...
def main() -> int:
    ...
    print("=" * 70)
    print("E2E Test — Peer Average CVC/CC: INVEX vs PROMEDIO")
    ...
    # -- Save results JSON --
    out = Path(__file__).parent / "peer_avg_cvc_cc_results.json"
    prompt_data = {}
