#!/usr/bin/env python3
"""
E2E Test — Tasa MN dual prompts: Ranking (bar chart) + Trend (line chart)

Tests the two onboarding presets for Tasa Promedio MN:
  1. Ranking: horizontal bar chart with individual bank rates + table
  2. Trend: INVEX vs PROMEDIO time series line chart

Tableau "Tasas Moneda Nacional" view has both components.

Usage:
    python tests/e2e/charts/test_tasa_mn_dual_prompts.py
"""
...
# ── Prompts (from onboarding presets) ────────────────────────────────────────
...
PROMPT_TREND = (
    "Crea una gráfica donde se compare la tasa promedio en Moneda Nacional de INVEX "
...
# Tableau reference for MN — values are higher than ME (typically 10-20%)
TABLEAU_BANKS_MN = {
    "BANCA MIFEL": 16.57,
    "INVEX": 14.53,
    "AFIRME": 13.88,
...
def _r4_values_percentage(resp: dict[str, Any]) -> tuple[bool, str]:
    """R4: Values should be percentage range (5-25% for MN rates)."""
    traces = _get_traces(resp)
    if not traces:
        return False, "NO_TRACES"
    for t in traces:
        orientation = t.get("orientation", "v")
        vals = t.get("x", []) if orientation == "h" else t.get("y", [])
        numeric = [v for v in (vals or []) if isinstance(v, (int, float)) and v > 0]
        if numeric:
            max_v = max(numeric)
            min_v = min(numeric)
            if max_v < 1.0:
...
def _r6_table_in_text(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    has_bank_values = sum(1 for b in TABLEAU_BANKS_MN if b.upper() in content.upper()) >= 3

    if has_pipe_table:
        return True, "Table detected (pipe format)"
    if has_bank_values:
        return True, "Bank names found in text (implicit table)"
...
def _t4_period_coverage(resp: dict[str, Any]) -> tuple[bool, str]:
    """T4: Should span multiple years from 2017."""
    traces = _get_traces(resp)
    all_dates: list[str] = []
    for t in traces:
...
def _t5_values_pct(resp: dict[str, Any]) -> tuple[bool, str]:
    """T5: Values should be percentages (5-25% for MN rates)."""
...
TREND_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck("T4_PERIOD", "Multi-year span from 2017", _t4_period_coverage),
...
def main() -> int:
    print("=" * 70)
    print("E2E Test — Tasa MN: Dual Prompts (Ranking + Trend)")
    ...
    # ── Save results ──
    out = Path(__file__).parent / "tasa_mn_dual_results.json"
    out.write_text(json.dumps({
        "test": "tasa-mn-dual-prompts",
