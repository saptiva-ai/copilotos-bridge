#!/usr/bin/env python3
"""
E2E Test — ICAP Snapshot Bar Chart (Índice de Capitalización)

Tests the onboarding prompt for "Ranking ICAP" which asks for a
single-period horizontal bar chart of ICAP across 10 banks.

Tableau reference view: "ICAP — Índice de Capitalización — 01/2025"

Pipeline under test:
  evolucion_banco_handler._handle_hip_snapshot → execute_hip_snapshot()
  (hip_icap registered in _METRIC_MAP → bypasses NL2SQL)
...
Usage:
    python tests/e2e/charts/test_icap_snapshot_bar_chart.py
"""
...
# Tableau reference values for 01/2025 (ICAP = Índice de Capitalización)
# ICAP is stored as percentage: 16.08 means 16.08%
TABLEAU_REFERENCE = {
    "BANCO BASE": 22.61,
    "MONEX": 18.95,
    "MIFEL": 16.58,
...
def v7_values_plausible(data: dict) -> str | None:
    """ICAP percentage should be between 5 and 40 for these banks."""
    chart = data.get("bank_chart", {})
    traces = chart.get("plotly_config", {}).get("data", [])
    if not traces:
        return "No traces"
    ...
    out_of_range = [v for v in valid if v < 5 or v > 40]
    if out_of_range:
        return f"Values out of range [5, 40]: {out_of_range}"
    return None


def v8_tableau_accuracy(data: dict) -> str | None:
    """Check that values match Tableau reference within +/-0.5 pp."""
    chart = data.get("bank_chart", {})
    traces = chart.get("plotly_config", {}).get("data", [])
    if not traces:
        return "No traces"
    ...
    if mismatches:
        return f"Mismatches (>0.5 pp): {'; '.join(mismatches)}"
    return None
...
def main():
    ...
    checks = [
        ...
        _run("V7_VALUES_PLAUSIBLE", "ICAP values between 5 and 40", v7_values_plausible, resp),
        _run("V8_TABLEAU_ACCURACY", "Values match Tableau (+-0.5 pp)", v8_tableau_accuracy, resp),
    ...
    print(f"\n{'=' * 70}")
    print(f"TEST: ICAP Snapshot Bar Chart (Índice de Capitalización)")
    print(f"{'=' * 70}\n")
    ...
    out_path = Path(__file__).with_name("icap_snapshot_results.json")
