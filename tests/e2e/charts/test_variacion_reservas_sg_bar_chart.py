#!/usr/bin/env python3
"""
E2E Test — Variacion de Reservas Sin Gobierno: Bar Chart Pipeline

Sends the exact multi-bank Reservas SG (sin entidades gubernamentales)
...
table_data, metric detection (reservas_sg), text/chart coherence, and
anti-fabrication guards.

This test validates the pipeline that feeds through:
  evolucion_banco_handler._detect_metric()  → "reservas_sg" (or similar)
...
Prompt under test:
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025.
     Compara las reservas sin reservas de entidades gubernamentales entre el
     periodo inicial y el periodo final entre los bancos: MONEX, INVEX,
     BANCREA, SABADELL, BANCA MIFEL, MULTIVA, AFIRME, BANSI, VE POR MÁS
     Y BANCO BASE. Presenta el dato del periodo inicial, el dato del periodo
     final y la variacion entre el periodo inicial.
     Donde la variacion es = (periodo actual / periodo inicial -1)
     Haz una gráfica de barras donde se vea la variación graficada y marca
     a INVEX de color rojo. Así como una tabla con:
     Banco | Reservas SG 2024 | Reservas SG 2025 | % Variación"

Usage:
    python tests/e2e/charts/test_variacion_reservas_sg_bar_chart.py
"""
...
# The exact prompt — "reservas sin reservas de entidades gubernamentales"
PROMPT = (
    "Toma como periodo inicial enero 2024 y como periodo actual enero 2025. "
    "Compara las reservas sin reservas de entidades gubernamentales entre el "
    "periodo inicial y el periodo final entre los bancos: MONEX, INVEX, "
...
def _check_metric_detection(resp: dict[str, Any]) -> tuple[bool, str]:
    """V3: Metric must be reservas SG (sin gobierno), not reservas_totales or cartera.

    The prompt says "reservas sin reservas de entidades gubernamentales",
    so the handler should detect a reserves-without-government metric,
    NOT reservas_totales or any cartera variant.
    """
    bc = resp.get("bank_chart")
    if not bc:
    ...
    # Positive: "reserva" should appear somewhere
    has_reservas = "reserva" in combined

    # Strong positive: "sin gobierno" or "sg" qualifier present
    has_sg_qualifier = (
        "sin gobierno" in combined
        or "sin gob" in combined
        or "_sg" in combined
        or "sin entidades gubernamentales" in combined
    )

    # Negative: should NOT say "cartera" (wrong metric family)
    has_cartera = "cartera" in combined

    if has_reservas and has_sg_qualifier:
        return True, (
            f"Metric detection OK: 'reservas' + SG qualifier found. "
            f"title='{title}', summary='{summary[:60]}'"
        )

    if has_reservas and not has_cartera:
        return True, (
            f"METRIC_PARTIAL: 'reservas' found but no explicit SG qualifier. "
            f"title='{title}', summary='{summary[:60]}'"
        )

    if has_cartera:
        return False, (
            f"WRONG_METRIC: detected cartera instead of reservas SG. "
            f"_detect_metric() matched the wrong keyword. "
            f"title='{title}'"
        )

    # Fallback: check content text
    content_lower = (resp.get("content") or "").lower()
    if "reserva" in content_lower:
        metric_type = metadata.get("metric_type", "")
        return True, (
            f"METRIC_IN_CONTENT: 'reserva' found in LLM text. "
            f"metric_type='{metric_type}', title='{title[:50]}'"
        )

    metric_type = metadata.get("metric_type", "")
    return True, (
        f"METRIC_INDIRECT: no explicit 'reserva' in title/summary, "
...
def _check_table_data(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    return True, (
        ...
        f"{valid_rows} with valid variation. Columns: {columns}"
    )
...
ALL_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck(
        ...
        description=(
            "_detect_metric() must classify 'reservas sin entidades "
            "gubernamentales' as reservas_sg (not reservas_totales or cartera)"
        ),
...
# ══════════════════════════════════════════════════════════════════════════════
# Runner (replay pattern from test_variacion_cartera_comercial_bar_chart.py)
# ══════════════════════════════════════════════════════════════════════════════
...
def main() -> int:
    print("=" * 70)
    print("E2E Test — Variacion de Reservas SG: Bar Chart Pipeline")
    ...
    # ── Save results JSON ──
    out = Path(__file__).parent / "variacion_reservas_sg_results.json"
    out.write_text(
        json.dumps(
            {
                "test": "variacion-reservas-sg-bar-chart",
                "prompt": PROMPT,
                "backend_url": BACKEND_URL,
                "total_checks": total,
                "passed": passed,
