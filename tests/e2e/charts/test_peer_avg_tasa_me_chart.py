#!/usr/bin/env python3
"""
E2E Test — Peer Average Tasa Promedio ME: INVEX vs PROMEDIO
...
Tasa Promedio en Moneda Extranjera (Tableau "Tasas Moneda Extranjera" view).

Values are interest rate percentages (typically 2-16%).

Prompts under test:
    A) "...tasa promedio en Moneda Extranjera de INVEX contra ... bancos: MONEX, ... De enero 2017..."
    B) "...tasa promedio en Moneda Extranjera de INVEX contra ... bancos (MONEX, ...) de enero 2017..."
    C) "...de enero 2017... tasa promedio en Moneda Extranjera de INVEX contra ... bancos: MONEX, ..."

Usage:
    python tests/e2e/charts/test_peer_avg_tasa_me_chart.py
"""
...
# Tableau reference values for Tasa ME (01/2025)
TABLEAU_REFERENCE = {
    "BANCA MIFEL": 15.29,
    "INVEX": 9.05,
    "AFIRME": 8.22,
    ...
    "MONEX": 7.12,
    "BANCREA": 2.95,
    # BANSI and MULTIVA may not have tasa_me data
}

# ── Prompt variants ──────────────────────────────────────────────────────────
# Variant A: user's natural prompt — colon after "bancos", period at end.
PROMPT_A_COLON_END = (
    "Crea una gráfica donde se compare la tasa promedio en Moneda Extranjera "
...
PROMPT_C_COLON_START = (
    ...
    "tengas donde se compare la tasa promedio en Moneda Extranjera de INVEX "
...
def _check_period_coverage(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    Note: tasa_me data may start later than 2017 for some banks.
    We accept any start date <= 2020-06 and require recent data >= 2024.
    """
    traces = _get_traces(resp)
    if not traces:
    ...
    return False, (
        ...
        f"Expected start <= 2020-06 and end >= 2024."
    )
...
def _check_data_point_count(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    tasa_me data may be sparser than tasa_mn, so threshold is lower.
    """
    traces = _get_traces(resp)
    if not traces:
...
def _check_values_are_percentage(resp: dict[str, Any]) -> tuple[bool, str]:
    """V6: Y-values should be interest rate percentages (typically 2-20%)."""
    traces = _get_traces(resp)
    if not traces:
        return False, "NO_TRACES"
    ...
    for t in traces:
        ...
        # Tasa ME should be in percentage range (2-20%), not raw ratio (0.02-0.20)
        if max_val < 1.0:
            return False, (
                f"RAW_RATIO: {name} max={max_val:.4f} — "
                f"values appear to be raw ratios, not percentages. "
                f"Expected x100 normalization."
            )
        if max_val > 50:
            return False, (
...
def _check_promedio_plausible(resp: dict[str, Any]) -> tuple[bool, str]:
    ...
    # PROMEDIO of ME interest rates should be in 2-20% range, not 100+%
    if max_val > 50:
        return False, (
...
def _check_layout_title(resp: dict[str, Any]) -> tuple[bool, str]:
    """V10: Chart title should reference 'tasa' or 'ME' or 'moneda extranjera'."""
    ...
    # Accept various title patterns for tasa ME
    has_tasa = "tasa" in title_lower
    has_me = "me" in title_lower or "moneda extranjera" in title_lower or "extranjera" in title_lower

    if has_tasa or has_me:
    ...
    return False, (
        ...
        f"title is too long or doesn't reference tasa/ME"
    )
...
def _parse_percentage_values(text: str) -> list[tuple[float, str]]:
    ...
    for m in pct_pat.finditer(text):
        try:
            val = float(m.group(1))
            if 1.0 <= val <= 25.0:  # Plausible ME interest rate
...
ALL_CHECKS: list[ComponentCheck] = [
    ...
    ComponentCheck(
        ...
        description="Data should span from at most 2020-06 to a recent date",
    ...
    ComponentCheck(
        ...
        description="Y-values must be interest rate percentages (2-20%), not raw ratios",
    ...
    ComponentCheck(
        ...
        description="Chart title should reference 'tasa' or 'ME' or 'moneda extranjera'",
...
def main() -> int:
    ...
    print("=" * 70)
    print("E2E Test — Peer Average Tasa Promedio ME: INVEX vs PROMEDIO")
    ...
    # ── Save results JSON ──
    out = Path(__file__).parent / "peer_avg_tasa_me_results.json"
    prompt_data = {}
