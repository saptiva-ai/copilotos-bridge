...
ssh user@server "cd /path && MONGODB_PASSWORD="${MONGODB_PASSWORD}" MONGODB_USER='user' ./backup-mongodb.sh"
```
...
#!/bin/bash
docker exec copilotos-prod-mongodb mongosh -u ${MONGODB_USER} -p '${MONGODB_PASSWORD}' --authenticationDatabase admin copilotos --quiet --eval 'print("Users: " + db.users.countDocuments())'
EOF
...
ssh user@server "cd /opt/copilotos-bridge && MONGODB_PASSWORD=\"${MONGODB_PASSWORD}\" MONGODB_USER=\"${MONGODB_USER}\" ./scripts/backup-mongodb.sh"
```
...
Then:
```bash
MONGODB_PASSWORD="${MONGODB_PASSWORD}" ssh prod-server './scripts/backup-mongodb.sh'
```
...
#   MONGODB_PASSWORD="${MONGODB_PASSWORD}" ./script.sh
#
#   # Option 3: Via SSH
